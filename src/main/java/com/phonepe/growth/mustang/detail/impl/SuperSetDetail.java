...
public class SuperSetDetail extends Detail {
    @NotEmpty
    @Singular
    ...
    @Builder
    @JsonCreator
    public SuperSetDetail(@JsonProperty("values") Set<Object> values) {
        super(Caveat.SUPERSET);
        this.values = values;
    }
    ...
    public boolean validate(Object lhsValue) {
        if (Objects.nonNull(lhsValue) && List.class.isAssignableFrom(lhsValue.getClass())) {
            return ((List<?>) lhsValue).containsAll(values);
        }
        return false;
    }
